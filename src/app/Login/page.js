...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (error) {
      ...
      // Si el inicio de sesión es exitoso, vamos al Dashboard
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-pink-200 via-pink-50 to-green-200 p-6 relative overflow-hidden ${poppins.className}`}>
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 md:p-12 w-full max-w-lg shadow-2xl text-center z-10 mt-16 md:mt-0">
        ...
        <h2 className={`${montserrat.className} text-3xl md:text-4xl font-black text-black mb-2`}>¡Te damos la bienvenida!</h2>
        <p className="text-gray-700 text-lg font-medium mb-8">Tu proceso te está esperando...</p>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5 mb-8 text-left">
          <div className="w-full">
            ...
            <div className="flex items-center bg-[#F4EBE0] rounded-full px-4 py-3 border border-transparent focus-within:border-teal-600 transition-all">
              <svg className="w-5 h-5 text-gray-600 mr-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
              <input 
                type="email" 
                required
                value={email}
          ...
          <div className="w-full">
            ...
            <div className="flex items-center bg-[#F4EBE0] rounded-full px-4 py-3 border border-transparent focus-within:border-teal-600 transition-all">
              <svg className="w-5 h-5 text-gray-600 mr-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path></svg>
              <input 
                type="password" 
                required
                value={password}
          ...
          <div className="mt-8">
            <button 
              type="submit" 
              disabled={cargando}
              className={`w-full flex justify-center text-white font-bold text-xl py-4 rounded-full transition-all duration-300 shadow-md ${cargando ? 'bg-teal-500 cursor-not-allowed' : 'bg-teal-700 hover:bg-teal-800 hover:shadow-xl hover:-translate-y-1'}`}
            >
